...
- **메인 페이지 이벤트 팝업** — 개발자 행사 등 홍보용 모달. 메인(`/`)에서만 노출되며 `site.config.js` 의 `eventPopup` 으로 토글/배너 이미지/타이틀/일시·장소/CTA 링크를 관리합니다. `image` 필드가 있으면 모달 상단에 풀폭 배너가 들어가고, 이미지 비율은 자동으로 맞춰집니다(16:9, OG 40:21, 1:1 등 자유). "오늘 하루 보지 않기" 누르면 24시간 동안 안 뜸.
