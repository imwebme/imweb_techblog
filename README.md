...
- **메인 페이지 이벤트 팝업** — 개발자 행사 등 홍보용 모달. 메인(`/`)에서만 노출되며 `site.config.js` 의 `eventPopup` 으로 토글/타이틀/일시·장소/CTA 링크를 관리합니다. "오늘 하루 보지 않기" 누르면 24시간 동안 안 뜸.
- **Google Analytics 4 (opt-in)** — 하단 동의 배너에서 "동의" 를 눌러야만 `gtag` 스크립트가 로드됩니다(PIPA 보수적 해석). "거부" 누르면 영구 미로드. 측정 ID 는 `site.config.js` 의 `analytics.measurementId` 에서 관리.
