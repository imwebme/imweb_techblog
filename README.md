...
>
> **multi-select 는 노션 옵션 목록이 기준입니다.** 노션에서 옵션을 지워도 이미 그 값을 쓰던 페이지에는 raw 문자열이 남아(노션 UI 에선 안 보임) 사이트에 유령 태그로 노출되던 문제가 있어, 옵션에 정의되지 않은 값은 걸러냅니다. 즉 **옵션을 지우면 사이트에서도 사라집니다.**
